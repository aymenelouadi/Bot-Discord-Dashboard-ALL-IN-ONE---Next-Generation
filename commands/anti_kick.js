...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(CMD)
        .setDescription('Anti-Kick settings | إعدادات منع الطرد الجماعي')
        .addIntegerOption(o => o
            .setName('limit')
            .setDescription('Max kicks before punishment | الحد قبل التدخل')
    ...
    textCommand: { name: 'antikick', aliases: ['anti-kick'] },
    ...
    async execute(client, ctx, args) {
        ...
        try {
            ...
            if (isSlash) {
                ...
                if (args.length < 2) {
                    const u = lang==='en'
                        ? '**Usage:** `!antikick <limit 1-20> <1/2/3> [on/off]`'
                        : '**الاستخدام:** `!antikick <الحد 1-20> <1/2/3> [on/off]`';
            ...
            await logSystem.logCommandUsage({ interaction: ctx, commandName: CMD, moderator, target: moderator,
                reason: `anti_kick → limit:${limit} action:${action} enabled:${enabled}`, action: CMD }).catch(()=>{});

            return sendResponse(ctx, isSlash, buildEmbed({
                guildId, protectionKey: CMD,
                titleAr: 'منع الطرد الجماعي', titleEn: 'Anti Mass-Kick',
