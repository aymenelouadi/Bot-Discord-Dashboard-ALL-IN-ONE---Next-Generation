...
const CMD = 'anti_channel_create';

module.exports = {
    data: new SlashCommandBuilder()
        .setName(CMD)
        .setDescription('Anti Channel-Create | منع إنشاء القنوات')
    ...
    textCommand: { name: 'antichcreate', aliases: ["anti-channel-create"] },
    ...
    async execute(client, ctx, args) {
        ...
        try {
            ...
            if (isSlash) {
                ...
                if (args.length < 2) {
                    const u = lang === 'en'
                        ? `**Usage:** \`!antichcreate <limit 1-20> <1/2/3> [on/off]\``
                        : `**الاستخدام:** \`!antichcreate <الحد 1-20> <1/2/3> [on/off]\``;
            ...
            await logSystem.logCommandUsage({
                ...
                reason: `anti_channel_create → limit:${limit} action:${action} enabled:${enabled}`,
                action: CMD
            }).catch(() => {});

            return sendResponse(ctx, isSlash, buildEmbed({
                guildId, protectionKey: CMD,
                titleAr: 'منع إنشاء القنوات', titleEn: 'Anti Channel-Create',
