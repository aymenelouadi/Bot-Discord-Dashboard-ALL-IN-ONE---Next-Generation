...
const CMD = 'anti_role_delete';

module.exports = {
    data: new SlashCommandBuilder()
        .setName(CMD)
        .setDescription('Anti Role-Delete | منع حذف الرتب')
    ...
    async execute(client, ctx, args) {
        ...
        try {
            ...
            if (isSlash) {
                ...
                if (args.length < 2) {
                    const u = lang === 'en'
                        ? `**Usage:** \`!antiroledelete <limit 1-20> <1/2/3> [on/off]\``
                        : `**الاستخدام:** \`!antiroledelete <الحد 1-20> <1/2/3> [on/off]\``;
            ...
            await logSystem.logCommandUsage({
                ...
                reason: `anti_role_delete → limit:${limit} action:${action} enabled:${enabled}`,
                action: CMD
            }).catch(() => {});

            return sendResponse(ctx, isSlash, buildEmbed({
                guildId, protectionKey: CMD,
                titleAr: 'منع حذف الرتب', titleEn: 'Anti Role-Delete',
